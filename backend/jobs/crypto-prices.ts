#!/usr/bin/env npx tsx
/**
 * Job: Ingest crypto prices from Twelve Data (5-day window).
 * Usage: npx tsx backend/jobs/crypto-prices.ts
 */
...
async function fetchCryptoPrice(ticker: string, apiKey: string): Promise<string> {
...
async function main(): Promise<void> {
  ...
  console.log(`[CryptoPrices] Starting: ${tickers.length} tickers`);

  let success = 0, errors = 0;
  for (const ticker of tickers) {
    try {
      const result = await fetchCryptoPrice(ticker, apiKey);
  ...
  console.log(`[CryptoPrices] Done: ${success} success, ${errors} errors`);
  if (errors > tickers.length * 0.5) process.exit(1);
}

main().catch((err) => { console.error('[CryptoPrices] Fatal:', err); process.exit(1); });
