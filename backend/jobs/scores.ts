#!/usr/bin/env npx tsx
/**
 * Job: Run technical + sentiment + fundamental + regime scoring + conclusions.
 * Usage: npx tsx backend/jobs/scores.ts
 */
...
interface CronConfig {
  ...
  promptSentiment: string; promptSentimentFilter: string; promptConclusion: string;
  sentimentLookbackDays: number; cryptoMinQualifying: number; sentimentDecayFactor: number;
  maxTokensSentiment: number; maxTokensConclusion: number; maxCharsConclusion: number;
  subweightMacd: number; subweightEma: number; subweightRsi: number;
  subweightBollinger: number; subweightVolume: number;
  subweightPe: number; subweightRevenue: number; subweightMargin: number;
  subweightRoe: number; subweightDebt: number;
}
...
async function runRegime(supabase: SB, dateStr: string): Promise<void> {
  const [spyCloses, xlkCloses, xlvCloses] = await Promise.all([
    fetchCloses(supabase, 'SPY', dateStr), fetchCloses(supabase, 'XLK', dateStr), fetchCloses(supabase, 'XLV', dateStr),
  ]);
  ...
  if (spyCloses.length >= 20) {
    ...
    const components = { spyTrendScore, volatilityScore: volScore, sectorRotationScore: sectorScore, regimeLabel: label, volatilityLevel: vol < 0.10 ? 'low' : vol < 0.20 ? 'moderate' : 'high', broadTrend: spyTrendScore > 0.2 ? 'uptrend' : spyTrendScore < -0.2 ? 'downtrend' : 'sideways', sectorRotation: sectorScore > 0.1 ? 'risk-on' : sectorScore < -0.1 ? 'risk-off' : 'balanced' };
  ...
  const [btcCloses, ethCloses] = await Promise.all([fetchCloses(supabase, 'BTC', dateStr), fetchCloses(supabase, 'ETH', dateStr)]);
  ...
  if (btcCloses.length >= 20) {
    ...
    const label = getRegimeLabel(regimeScore);
    const components = { btcTrendScore, volatilityScore: volScore, altSeasonScore: altScore, regimeLabel: label, volatilityLevel: vol < 0.40 ? 'low' : vol < 0.80 ? 'moderate' : 'high', broadTrend: btcTrendScore > 0.2 ? 'uptrend' : btcTrendScore < -0.2 ? 'downtrend' : 'sideways', sectorRotation: altScore > 0.1 ? 'risk-on' : altScore < -0.1 ? 'risk-off' : 'balanced' };
...
async function generateConclusions(supabase: SB, anthropic: Anthropic, dateStr: string, cfg: CronConfig): Promise<{ generated: number; errors: number }> {
  ...
  const [allScores, allNews, allAssets, allFunds, allQuotes] = await Promise.all([
    fetchAllScores(supabase, dateStr), fetchAllNews(supabase, todo),
    fetchAllAssets(supabase), fetchAllFundamentals(supabase), fetchAllQuotes(supabase),
  ]);
  ...
  for (let i = 0; i < todo.length; i += 5) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (ticker) => {
        ...
        const system = cfg.promptConclusion
          ? cfg.promptConclusion.replace(/\{\{name\}\}/g, name).replace(/\{\{ticker\}\}/g, ticker).replace(/\{\{type\}\}/g, type).replace(/\{\{max_chars\}\}/g, String(cfg.maxCharsConclusion))
    ...
    for (const r of results) {
      ...
      else { errors++; console.error('[Scores] Conclusion error:', r.reason); }
    }
...
async function main(): Promise<void> {
  ...
  console.log(`[Scores] Starting for ${dateStr}: ${tickers.length} tickers`);

  const [cfgStrings, cfgNumbers] = await Promise.all([
    getConfigBatch({
      model_sentiment: 'claude-haiku-4-5-20251001', model_sentiment_filter: 'claude-haiku-4-5-20251001',
    ...
    getConfigNumberBatch({
      sentiment_lookback_days: 10, sentiment_min_articles_crypto: 3, sentiment_decay_factor: 0.9,
      max_tokens_sentiment: 300, max_tokens_conclusion: 300, max_chars_conclusion: 450,
      subweight_technical_macd: 0.30, subweight_technical_ema: 0.25, subweight_technical_rsi: 0.20,
      subweight_technical_bollinger: 0.15, subweight_technical_volume: 0.10,
      subweight_fundamental_pe: 0.25, subweight_fundamental_revenue: 0.25,
      subweight_fundamental_margin: 0.15, subweight_fundamental_roe: 0.20, subweight_fundamental_debt: 0.15,
    }),
  ]);

  const cronCfg: CronConfig = {
    sentimentModel: cfgStrings['model_sentiment']!, sentimentFilterModel: cfgStrings['model_sentiment_filter']!,
    conclusionModel: cfgStrings['model_conclusion']!, promptSentiment: cfgStrings['prompt_sentiment']!,
    promptSentimentFilter: cfgStrings['prompt_sentiment_filter']!, promptConclusion: cfgStrings['prompt_conclusion']!,
    sentimentLookbackDays: cfgNumbers['sentiment_lookback_days']!, cryptoMinQualifying: cfgNumbers['sentiment_min_articles_crypto']!,
    sentimentDecayFactor: cfgNumbers['sentiment_decay_factor']!, maxTokensSentiment: cfgNumbers['max_tokens_sentiment']!,
    maxTokensConclusion: cfgNumbers['max_tokens_conclusion']!, maxCharsConclusion: cfgNumbers['max_chars_conclusion']!,
    subweightMacd: cfgNumbers['subweight_technical_macd']!, subweightEma: cfgNumbers['subweight_technical_ema']!,
    subweightRsi: cfgNumbers['subweight_technical_rsi']!, subweightBollinger: cfgNumbers['subweight_technical_bollinger']!,
    subweightVolume: cfgNumbers['subweight_technical_volume']!, subweightPe: cfgNumbers['subweight_fundamental_pe']!,
    subweightRevenue: cfgNumbers['subweight_fundamental_revenue']!, subweightMargin: cfgNumbers['subweight_fundamental_margin']!,
    subweightRoe: cfgNumbers['subweight_fundamental_roe']!, subweightDebt: cfgNumbers['subweight_fundamental_debt']!,
  };
  ...
  console.log('[Scores] Running market regime...');
  try { await runRegime(supabase, dateStr); } catch (e) { console.error('[Scores] Regime error:', e); }

  console.log('[Scores] Generating conclusions...');
